...
use std::{borrow::Cow, collections::HashMap, future::Future, net::IpAddr, sync::Arc};
...
struct DependencyAccessor<F, Fut>
where
    for<'a> F: Fn(FunctionID, Request<'a>) -> Fut + Clone + Send + Sync,
...
impl<F, Fut> Clone for DependencyAccessor<F, Fut>
where
    for<'a> F: Fn(FunctionID, Request<'a>) -> Fut + Clone + Send + Sync,
    Fut: Future<Output = Result<Response<'static>>>,
{
...
where
    for<'a> F: Fn(FunctionID, Request<'a>) -> Fut + Clone + Send + Sync + 'static,
    Fut: Future<Output = Result<Response<'static>>> + 'static,
{
    let (tx, rx) = NotificationChannel::<Notification>::new();

    let gateways = Arc::new(RwLock::new(HashMap::new()));

    let accessor: DependencyAccessor<F, Fut> = {
...
async fn handle_request<'a, F, Fut>(
    request: HttpRequest,
    payload: Option<web::Bytes>,
    dependency_accessor: web::Data<DependencyAccessor<F, Fut>>,
) -> ResponseWrapper
where
    for<'b> F: Fn(FunctionID, Request<'b>) -> Fut + Clone + Send + Sync,
    Fut: Future<Output = Result<Response<'static>>>,
{
