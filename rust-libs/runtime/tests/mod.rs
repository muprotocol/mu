...
async fn test_simple_func<'a>(
    ...
    let (runtime, db_manager, _assembly_provider, _usages) = runtime_fixture;
    let projects = create_and_add_projects(&[("hello-wasm", &["say_hello"], None)], &runtime)
        .await
        .unwrap();
